...
back to FALSE at shutdown by the page_cleaner as well. Therefore n87o
need to protect it by a mutex. It is only eve98iiiiiiiiiiiiiiiiiiiii6??????????????????????????????????????iii7yuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu/r read by the thread
doing the shutdown */
...
/* lbh */
#define PAGE_CLEANER_LRU_BATCH_CHUNK_SIZE	1000
	...
	buf_page_t*	bpage)	/*!< in: pointer to the block in question */
{
	...
	//fprintf(stderr, "1bpage: %lu, n_flush[BUF_FLUSH_LRU]: %lu\n", bpage, buf_pool->n_flush[BUF_FLUSH_LRU] );
	//fprintf(stderr, "2bpage: %lu, n_flush[BUF_FLUSH_LIST]: %lu\n", bpage, buf_pool->n_flush[BUF_FLUSH_LIST] );
	...
	bool		sync)		/*!< in: true if sync IO request */
{
	...
	if (!srv_use_doublewrite_buf || !buf_dblwr) {
		//fprintf(stderr, "fil_io() before, bpage:%lu\n", bpage);
		fil_io(OS_FILE_WRITE | OS_AIO_SIMULATED_WAKE_LATER,
		       sync, buf_page_get_space(bpage), zip_size,
		       buf_page_get_page_no(bpage), 0,
		       zip_size ? zip_size : UNIV_PAGE_SIZE,
		       frame, bpage);
			   //fprintf(stderr, "fil_io(), bpage:%lu\n", bpage);
	...
	if (sync) {
		if(flush_type==BUF_FLUSH_LRU){
			//fprintf(stderr, "sync true but tail flush!\n");
		}
	...
	bool		sync)		/*!< in: true if sync IO request */
{
	...
	ut_ad(buf_flush_ready_for_flush(bpage, flush_type));
		...
		if(flush_type==BUF_FLUSH_LRU){
			//fprintf(stderr, "buf_flush_page(), bpage:%lu, pending writes:%lu\n", bpage, buf_pool->n_flush[BUF_FLUSH_LRU]);
		...
		if (flush_type == BUF_FLUSH_LIST
		    && is_uncompressed
		    && !rw_lock_s_lock_gen_nowait(rw_lock, BUF_IO_WRITE) && !(bpage->LRU_batch_write_victim && !bpage->aio_write_finished)) {
					...
					we are allowed to flush */
{
	...
	if (UT_LIST_GET_LEN(buf_pool->LRU) < BUF_LRU_OLD_MIN_LEN
		...
		if (srv_flush_neighbors == 1) {
			fprintf(stderr,"flush neighbors also\n");
	...
	for (i = low; i < high; i++) {
		...
		if (flush_type != BUF_FLUSH_LRU
			...
			if(bpage->LRU_batch_write_victim && !bpage->aio_write_finished){
				mutex_exit(block_mutex);
			}else{
				if (buf_flush_ready_for_flush(bpage, flush_type)
					...
					++count;
					//fprintf(stderr, "buf_flush_try_neighbors(): count: %lu, bpage: %lu\n", count, bpage);
					...
					blocks in the free_list */
{
	...
	if(lru_len<free_len*3){
		//fprintf(stderr, "enough free list: \n");
		return(count);
	}
	...
	while (bpage != NULL && count < max
	       && free_len < srv_LRU_scan_depth
		   && bpage!=buf_pool->LRU_old
		   && !buf_pool->reached_LRU_oldest_clean_page
	       && lru_len > BUF_LRU_MIN_LEN
		   && scanned < buf_pool->LRU_old_len/2) {			
		...
		++scanned;
		//fprintf(stderr, "2scanned: %lu, free_len: %lu, lru_len: %lu, bpage:%lu\n", scanned, free_len, lru_len, bpage);
		...
		O(n*n). */
		if (evict) {
			//fprintf(stderr, "3scanned: %lu, buf_LRU_free_page()\n", scanned);
			...
			if (prev_bpage != NULL && buf_flush_ready_for_flush(prev_bpage, BUF_FLUSH_LRU)) {
			...
			if (!buf_flush_page_and_try_neighbors(
				...
				//fprintf(stderr, "3scanned: %lu, buf_flush_page_and_try_neighbors()\n", scanned);
					

				bpage = prev_bpage;
			} else {
				//fprintf(stderr, "4scanned: %lu, buf_flush_page_and_try_neighbors()\n", scanned);
	...
	//fprintf(stderr, "1scanned: %lu, free_len: %lu, lru_len: %lu\n", scanned, free_len, lru_len);
	...
	//fprintf(stderr, "buf_pool: %lu, buf_lru_flush_page_count: %lu, count: %lu\n",buf_pool, buf_lru_flush_page_count, count);
					...
					blocks in the free_list */
{
	...
	if (buf_LRU_evict_from_unzip_LRU(buf_pool)) {
		...
		//fprintf(stderr, "count: %lu in buf_do_LRU_batch(): buf_free_from_unzip_LRU_list_batch()\n", count);
	}
	
	/* lbh ver2 */
	//if (max > count && !buf_pool->reached_LRU_oldest_clean_page) {
	//if (max > count)

	if (buf_pool->reached_LRU_oldest_clean_page==FALSE) {
		count += buf_flush_LRU_list_batch(buf_pool, max - count);
		//fprintf(stderr, "count: %lu in buf_do_LRU_batch()\n", count);
	}
					...
					min_n) */
{
	...
	fprintf(stderr, "buf_do_flush_list_batch() start\n");
	...
	for (buf_page_t* bpage = UT_LIST_GET_LAST(buf_pool->flush_list);
		...
		//fprintf(stderr, "buf_do_flush_list_batch() bpage: %lu\n", bpage);
		...
		if (!buf_flush_is_hp(buf_pool, prev)) {
			//fprintf(stderr, "buf_do_flush_list_batch() bpage: %lu, buf_flush_is_hp()\n", bpage);
					...
					min_n), otherwise ignored */
{
	...
	case BUF_FLUSH_LRU:
		...
		buf_LRU_dirty_tail_list_mutex_exit(buf_pool);
		//fprintf(stderr, "in buf flush batch, count: %lu\n", count);
				
		
		break;
	case BUF_FLUSH_LIST:
		//fprintf(stderr,"flush list\n");		
		count = buf_do_flush_list_batch(buf_pool, min_n, lsn_limit);
		//fprintf(stderr, "after flush list soon release mutex\n");	
		break;
...
{
	//fprintf(stderr, "before buf_dblwr_flush_buffered_writes()\n");
	buf_dblwr_flush_buffered_writes();
	//fprintf(stderr, "after buf_dblwr_flush_buffered_writes()\n");
					...
					or BUF_FLUSH_LIST */
{
	...
	if(flush_type==BUF_FLUSH_LRU){
		buf_LRU_dirty_tail_list_mutex_enter(buf_pool);
		//fprintf(stderr, "buf_flush_start()\n");
	}else{
		buf_pool_mutex_enter(buf_pool);
	}
	...
	|| buf_pool->init_flush[flush_type] == TRUE 
		...
		/* There is already a flush batch of the same type running */
			...
			if(flush_type==BUF_FLUSH_LRU){
				...
				buf_LRU_dirty_tail_list_mutex_exit(buf_pool);
				//fprintf(stderr, "buf_flush_start() batch not finished\n");
			}else{
				buf_pool_mutex_exit(buf_pool);
			}
					...
					or BUF_FLUSH_LIST */
{
	...
	if(flush_type==BUF_FLUSH_LRU){
		//fprintf(stderr, "buf_flush_end() mutex enter\n");
		buf_LRU_dirty_tail_list_mutex_enter(buf_pool);
		//fprintf(stderr, "buf_flush_end() mutex enter1\n");
	}else{
		buf_pool_mutex_enter(buf_pool);
	}
	...
	if (buf_pool->n_flush[flush_type] == 0) {
		...
		os_event_set(buf_pool->no_flush[flush_type]);
		//fprintf(stderr, "buf_flush_end() os_event_set after\n");
		if(flush_type==BUF_FLUSH_LRU){
			buf_pool->batch_running = false;
			os_event_set(buf_pool->b_event);
			//fprintf(stderr, "buf_flush_end() os_event_set after b_event\n");
		}
	}

	if(flush_type==BUF_FLUSH_LRU){
		buf_LRU_dirty_tail_list_mutex_exit(buf_pool);
		//fprintf(stderr, "buf_flush_end() mutex exit\n");
	}else{
		buf_pool_mutex_exit(buf_pool);
	}
}
		...
		ulint	i;
			...
			for (i = 0; i < srv_buf_pool_instances; ++i) {
				...
				if (type==BUF_FLUSH_LRU) {
					...
					thd_wait_end(NULL);
					//fprintf(stderr, "buf_flush_wait_batch_end() thd_wait_end(NULL))\n");
					...
					back to caller. Ignored if NULL */
{
	...
	if (n_processed) {
		*n_processed = page_count;
		//fprintf(stderr, "1n_processed:%lu, page_count:%lu in buf_flush_LRU()\n", *n_processed, page_count);
		
	}
	//fprintf(stderr, "2n_processed:%lu, page_count:%lu in buf_flush_LRU()\n", *n_processed, page_count);
...
/*====================*/
{
	...
	for (ulint i = 0; i < srv_buf_pool_instances; i++) {
		...
		if(lru_len<free_len*2){
			//fprintf(stderr, "enough free page: lru_len: %lu, free_len:%lu\n", lru_len, free_len);
			buf_LRU_dirty_tail_list_mutex_exit(buf_pool);
			continue;
		}
		
		//fprintf(stderr, "lru_len: %lu, free_len:%lu\n", lru_len, free_len);
		...
		buf_LRU_dirty_tail_list_mutex_exit(buf_pool);
		buf_page_t*	prev_bpage;
		ulint 	n_clean_page = 0;

		
		while (bpage != NULL 
			&& scanned < UT_LIST_GET_LEN(buf_pool->LRU) / 8
		   && bpage!=buf_pool->LRU_old
	...
	for (ulint i = 0; i < srv_buf_pool_instances; i++) {
		...
		if (buf_pool->n_flush[BUF_FLUSH_LRU] > 0
		   || buf_pool->init_flush[BUF_FLUSH_LRU]) {
			...
			//fprintf(stderr, "buf_flush_wait_LRU_batch_end()");
			...
			os_thread_create */
{
	...
	// mutex enter and exit in buf_flush_wait_LRU_batch_end()
