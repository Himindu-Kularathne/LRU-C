...
/* lbh */
/******************************************************************//**
Keep record of oldest clean page per buffer pool instances. The purpose of this 
function is to decrease the LRU scanning overhead during buf_LRU_get_free_block()
@return	TRUE if properly updated */
UNIV_INTERN
ibool
buf_update_oldest_clean_page(
/*========================*/
	buf_pool_t*	buf_pool,	/*!< in: buffer pool instance */
					...
					'old' blocks. */
{
	...
	ut_ad(buf_pool_mutex_own(buf_pool)|| buf_LRU_dirty_tail_list_mutex_own(buf_pool));
	
	buf_page_t*	bpage;
	ulint		scanned;


	if(buf_pool->LRU_oldest_clean_page==NULL || UT_LIST_GET_PREV(LRU, buf_pool->LRU_oldest_clean_page) == buf_pool->LRU_old || scan_all || UT_LIST_GET_PREV(LRU, buf_pool->LRU_oldest_clean_page)==NULL ||UT_LIST_GET_PREV(LRU, UT_LIST_GET_PREV(LRU, buf_pool->LRU_oldest_clean_page))==NULL || UT_LIST_GET_PREV(LRU, UT_LIST_GET_PREV(LRU, buf_pool->LRU_oldest_clean_page))==buf_pool->LRU_old ){
		scan_all= true;
		buf_LRU_dirty_tail_list_mutex_enter(buf_pool);
		bpage = UT_LIST_GET_LAST(buf_pool->LRU);
		
	}else{
		bpage = UT_LIST_GET_PREV(LRU, buf_pool->LRU_oldest_clean_page);
	}
	if(!buf_page_in_file(bpage)){
		fprintf(stderr, "bpage not in file\n");
		return FALSE;
  	}
	...
	&& buf_pool!=NULL
	&& UT_LIST_GET_LEN(buf_pool->LRU) != 0){

		for (bpage,
			scanned = 1;
			bpage != buf_pool->LRU_old && bpage != NULL && buf_page_in_file(bpage) && bpage!=UT_LIST_GET_PREV(LRU, buf_pool->LRU_old) &&  buf_pool->flush_list_hp != bpage;
			++scanned) {
			...
			if(scanned>buf_pool->LRU_old_len){
				break;
			}
			...
			&& buf_page_get_io_fix(bpage)==BUF_IO_NONE 
			&& buf_page_in_file(bpage)
			&& !(bpage->LRU_batch_write_victim && !bpage->aio_write_finished)
			&& buf_page_is_old(bpage)
			&& buf_pool->flush_list_hp != bpage)
			|| (buf_pool->LRU_oldest_clean_page == bpage) ){	
				//fprintf(stderr, "\n%lu :not clean page failed, scanned: %lu\n", bpage, scanned);						
	...
	if(buf_pool->LRU_oldest_clean_page != bpage || !buf_page_in_file(buf_pool->LRU_oldest_clean_page) || !buf_flush_ready_for_replace(buf_pool->LRU_oldest_clean_page) ||buf_page_get_io_fix(buf_pool->LRU_oldest_clean_page) || !buf_flush_ready_for_replace(buf_pool->LRU_oldest_clean_page) || buf_page_get_io_fix(buf_pool->LRU_oldest_clean_page)!=BUF_IO_NONE || buf_pool->LRU_oldest_clean_page->buf_fix_count != 0 ){
		//fprintf(stderr, "buf_update_oldest_clean_page() failed: buf_pool: %lu, oldest: %lu, cur_page: %lu, scanned: %lu\n", buf_pool, buf_pool->LRU_oldest_clean_page, bpage, scanned);
		if(scan_all){
		...
		if(scan_all){
			...
			//fprintf(stderr, "buf_update_oldest_clean_page() succeed: buf_pool: %lu, oldest: %lu, cur_page: %lu, scanned: %lu\n", buf_pool, buf_pool->LRU_oldest_clean_page, bpage, scanned);
	}
	return TRUE;
}
...
// FOR LRU-C 
/******************************************************************//**
Check whether oldest clean page is valid in buffer pool instance. The purpose of this 
function is check whether oldest clean page is not dirty or not freed.
@return	TRUE if oldest_clean_page is still clean.*/
UNIV_INTERN
ibool
buf_oldest_clean_page_is_valid(
/*========================*/
	buf_pool_t*	buf_pool)	/*!< in: buffer pool instance */
{
	ut_ad(buf_pool_mutex_own(buf_pool));
	...
	/* lbh */
	buf_page_t* clean_page = buf_pool->LRU_oldest_clean_page;
	//fprintf(stderr, "oldest clean page: %lu\n", clean_page);

	if(clean_page != NULL
		&& (clean_page->oldest_modification == 0)
		&& (clean_page->buf_fix_count == 0 )
		&& (buf_page_get_io_fix(clean_page) == BUF_IO_NONE)
		&& UT_LIST_GET_LEN(buf_pool->LRU) != 0
		&& buf_page_in_file(clean_page)
		&& !(clean_page->LRU_batch_write_victim && !clean_page->aio_write_finished)){
			ut_ad(clean_page->in_LRU_list)
					...
					be interrupted */
{
	...
	case BUF_REMOVE_FLUSH_WRITE:
		ut_a(trx != 0);
		fprintf(stderr, "buf_LRU_remove_pages()\n");
...
loop:
	...
	if (n_iterations > 30) {
		//fprintf(stderr, "n_iterations: %lu\n", n_iterations);
				...
				compressed page of an uncompressed page */
{
	...
	if (!buf_page_can_relocate(bpage)|| !buf_flush_ready_for_replace(bpage) || !buf_page_in_file(bpage)) {
	...
	if (zip || !bpage->zip.data) {
		...
		if (bpage->oldest_modification) {
            ...
            if (!bpage->LRU_batch_write_victim) {
			    goto func_exit;
				//fprintf(stderr, "victim and also a dirty page\n");
            }
            /* end */
		}
...
func_exit:
	...
	if(buf_pool->LRU_oldest_clean_page == bpage){
		...
		buf_update_oldest_clean_page(buf_pool, false);
	...
	buf_block_t*	block)	/*!< in: block, must not contain a file page */
{
	...
	/* lbh */
	// FOR LRU-C 
    if ((&block->page)->LRU_batch_write_victim && (&block->page)->aio_write_finished) {
        (&block->page)->LRU_batch_write_victim = false;
	}
	/* lbh */
