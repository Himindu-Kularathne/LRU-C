...
UNIV_INTERN mysql_pfs_key_t	LRU_dirty_tail_list_mutex_key;
	...
	ulint		instance_no)	/*!< in: id of the instance */
{
	...
	mutex_create(flush_list_mutex_key, &buf_pool->flush_list_mutex,
		     SYNC_BUF_FLUSH_LIST);
	mutex_create(LRU_dirty_tail_list_mutex_key, &buf_pool->LRU_dirty_tail_list_mutex,
		     SYNC_BUF_LRU_DIRTY_TAIL_LIST);
	...
	buf_pool->reached_LRU_oldest_clean_page = false;
...
corrupt:
	...
	case BUF_IO_READ:
		...
		if (uncompressed) {
			...
			if( bpage->LRU_batch_write_victim){
				//fprintf(stderr,"buf_page_io_complete() IO_READ\n");
				bpage->aio_write_finished = true;
			}
	...
	case BUF_IO_WRITE:
		...
		if(bpage->LRU_batch_write_victim && !bpage->aio_write_finished ){
				//bpage->LRU_batch_write_victim = false;
				//fprintf(stderr,"buf_page_io_complete() IO_WRITE\n");
				bpage->aio_write_finished = true;
		}
